...
ShaderProgram::ShaderProgram(OpenGLContext *context)
    : vertShader(), fragShader(), prog(),
      ...
      unifModel(-1), unifModelInvTr(-1), unifViewProj(-1), unifColor(-1), unifPostType(-1),
      attrPosOffset(-1), unifLightDir(-1),
      context(context)
{}
...
void ShaderProgram::create(const char *vertfile, const char *fragfile)
{
    ...
    unifSampler2D     = context->glGetUniformLocation(prog, "u_sampler");
    unifLightViewProj = context->glGetUniformLocation(prog, "u_lightViewProj");
    unifShadowSampler = context->glGetUniformLocation(prog, "u_shadowMap");
    unifCamPos        = context->glGetUniformLocation(prog, "cameraPos");
    unifLightPos      = context->glGetUniformLocation(prog, "lightPos");
    unifNoiseSampler  = context->glGetUniformLocation(prog, "noise_sampler");
...
{
    useMe();
    if (d.opaqueElemCount() == -1)
    {
        throw std::out_of_range("drawOpaque wrong!");
    }
    ...
    if(d.bindOpaque())
    {
        ...
        if (attrUV != -1)
        {
            ...
            context->glVertexAttribPointer(attrUV, 4, GL_FLOAT, false, 12 * sizeof(float), (void*)(8 * sizeof(float)));
        }
    }


    d.bindOpaqueIdx();
    context->glDrawElements(d.drawMode(), d.opaqueElemCount(), GL_UNSIGNED_INT, 0);
    if (attrPos != -1) context->glDisableVertexAttribArray(attrPos);
    if (attrNor != -1) context->glDisableVertexAttribArray(attrNor);
...
{
    useMe();
    if (d.translElemCount() == -1)
    {
        throw std::out_of_range("drawTranslucent wrong!");
    }


    if(d.bindTranslucent())
    {
        if (attrPos != -1)
        {
            context->glEnableVertexAttribArray(attrPos);
    ...
    d.bindTranslucentIdx();
    context->glDrawElements(d.drawMode(), d.translElemCount(), GL_UNSIGNED_INT, 0);
    if (attrPos != -1) context->glDisableVertexAttribArray(attrPos);
    if (attrNor != -1) context->glDisableVertexAttribArray(attrNor);
    if (attrUV != -1) context->glDisableVertexAttribArray(attrUV);
    context->printGLErrorLog();
...
void ShaderProgram::setTerrainSampler(int textureSlot)
{
    useMe();
    if(unifSampler2D != -1)
    {
...
void ShaderProgram::setLightViewProjMatrix(const glm::mat4 &vp)
{
    useMe();
...
void ShaderProgram::setShadowSampler(int slot)
{
    useMe();
    if(unifShadowSampler != -1){
        context->glUniform1i(unifShadowSampler, slot);
    }
}


void ShaderProgram::setLightPos(glm::vec3 pos)
{
    useMe();
    if(unifLightPos != -1){
        context->glUniform3fv(unifLightPos, 1, &pos[0]);
    }
}

void ShaderProgram::setLightDir(glm::vec4 dir)
{
    useMe();
    if(unifLightDir != -1){
        context->glUniform4fv(unifLightDir, 1, &dir[0]);
    }
}
...
void ShaderProgram::setNoiseSampler(int slot)
{
    useMe();
    if(unifNoiseSampler != -1){
        context->glUniform1i(unifNoiseSampler, slot);
    }
}
...
void ShaderProgram::setPostType(int type)
{
    ...
    if(unifPostType != -1)
    {
        context->glUniform1i(unifPostType, type);
    }
}
...
void ShaderProgram::drawArray(Drawable &d, int slot)
{
    useMe();
